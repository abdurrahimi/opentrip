...
(function($) {
        ...
        if(product_id !== null && sale_price !== null){
            $.ajax({
                ...
                success: function(data){
                    ...
                    $("#discount_text").text('Rp.'+price(res.discount_amount));
                    $("#discount").text(res.discount_amount);
                    $("#total-pembelian").text('Rp'+price(res.total_price-res.discount_amount));
                    $("#transaksi-item").find("input[type=text], input[type=number]").val("0");
                    $el.faLoading(false);
                    console.log(res);

                    var url2 =  $("base").attr("url") + 'penjualan/apriori2/' + $("#transaksi_product_id").val();
                    $.ajax({
                        url: url2,
                        type: 'GET',
                        dataType: 'JSON',
                    ...
                    .done(function(e) {
                        if (e.kata != "") {
                            if (confirm(e.kata)) {
                                var person = prompt("Masukkan qty order untuk item tersebut: ", "1");
                                if (person == null || person == "" || person == 0 ) {
                                    alert("Qty tidak boleh kosong, penambahan item dibatalkan.");
                                } else {
                                    console.log(e.produk.id);
                                    $("#transaksi_category_id").val(e.produk.category_id);
                                    $("#transaksi_product_id").val(e.produk.id_produk);
                                    $("#jumlah").val(person);
                                    $("#sale_price").html("");
                                    $("#sale_price").html($("#sale_price").html()+"<option value='"+e.produk.sale_price+"'>"+e.produk.sale_price+"</option>" );
                                    $("#sale_price").val(e.produk.sale_price);
                                    $("#tambah-barang").click();
                                }
                            } else {
    ...
    $("#submit-transaksi").on('click',function(e){
        ...
        let transaction_id = $("#kode_transaksi").val();
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if(response.status == "ok"){
                        ...
                        console.log($("base").attr("url") + "penjualan/detail/"+penjualan[2].sales_id);
                       window.location.href = $("base").attr("url") + "penjualan/detail/"+penjualan[2].sales_id;
    ...
    ** Retur Penjualan
    ...
    $(document).ready(function() {
        ...
        $(".btnKirimReview").click(function(){
            let arr="";
            arr = {"rating": $(".myRatingStar").val(),"review":$("#myReview").val()};
            //arr['rating'] = $(".myRatingStar").val();
            //arr['review'] = $("#myReview").val();
            $.ajax({
                url: $("base").attr("url") + "penjualan/save_review/"+$("#mySalesId").val(),
                data: arr,
                type: 'POST',
                beforeSend: function () {
                    $el.faLoading();
                ...
                success: function (data) {
                    ...
                    $el.faLoading(false);
                    if(response.status == "ok"){
                        alert("Terimakasih atas review kamu");
                        window.location.href = $("base").attr("url") + "penjualan/review/"+$("#mySalesId").val();
                    }else if(response.status == "limit"){
                        alert("Stok jumlah produk yang anda pilih sudah habis");
                    }else{
                        alert("Terjadi error di server, silahkan coba lagi");
...
$(document).ready(function() {
    $("#customer_id").change(function(event) {
        $(".alertdiscount").fadeOut('fast', function() {});
        $("#discount_amount").val("0");
        var _customer = $("#customer_id").val();
        $.ajax({
            url: $("base").attr("url") + 'penjualan/check_customer_discount/' + _customer,
            type: 'GET',
            dataType: 'JSON',
            async: true,
            processData: false,
        ...
        .done(function(e) {
            if (e.result == 1) {
                $(".alertdiscount").fadeIn('fast', function() {});
                $("#discount_amount").val("10");
            }else{
                $(".alertdiscount").fadeOut('fast', function() {});
            }
        })
